...
import org.wikimedia.wikihadoop.StreamWikiDumpInputFormat;

import de.l3s.content.mapred.WikipediaPageInputFormat;

import edu.umd.cloud9.collection.wikipedia.WikipediaPage;


/**
 * Tool for indexing and temporal extraction of a particular Wikipedia XML dump file. This program keeps
 ...
 *
 */
public class WikiTimex extends Configured implements Tool {
	private static final Logger LOG = Logger.getLogger(WikiTimex.class);
   
	static InterpreterP inp = new InterpreterP();
	private static enum PageTypes {
		TOTAL, REDIRECT, DISAMBIGUATION, EMPTY, ARTICLE, STUB, OTHER
	};
	/**
  private static class TMapper extends Mapper<LongWritable, WikipediaPage, ImmutableBytesWritable, IntWritable> {
    @Override
	...
	private static class TMapper extends Mapper<Text, Text, ImmutableBytesWritable, IntWritable> {

		@Override
		public void map(Text key, Text p, Context context)
		throws IOException, InterruptedException {
			context.getCounter(PageTypes.TOTAL).increment(1);
			
			inp.execfile("revision_differ.py");
	...
	public static void main(String[] args) throws Exception {
		ToolRunner.run(new WikiTimex(), args);
	}
}
